...
export function generateReportData(params: {
  ...
  return {
    ...
    generatedSummary: `You are currently profiled as a ${params.style}. Your strongest indicators are ${strongest.map((item) => item.label.toLowerCase()).join(" and ")}, while the next jump will come from sharper ${categoryFocus[weakest[0].key]}.`,
    ...
    weaknesses: weakest.map((category) => `Improve ${category.label.toLowerCase()} consistency under pressure`),
    trainingPriorities: weakest.map((category) => categoryFocus[category.key]),
    coachingInsight: `Your best performance appears when ${strongest[0].key === "positioning" ? "controlling the pace near the net" : `your ${strongest[0].label.toLowerCase()} habits stay repeatable`}. Awareness must stay ahead of random aggression.`,
    roadmap: {
      weekOneTwo: `Movement base: ${categoryFocus[weakest[0].key]}.`,
      weekThreeFour: `Pressure layer: ${categoryFocus[weakest[1]?.key ?? weakest[0].key]}.`
    },
    justifications
  };
}
